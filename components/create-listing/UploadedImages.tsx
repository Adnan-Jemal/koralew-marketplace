...
export default function UploadedImages({ setImages, images,setError }: propTypes) {
  return (
        ...
        {images.map((img) => {
          ...
          return (
            <div key={img.name} className="relative">
              <Image
                className={`rounded-md select-none ${!url&&'animate-pulse'} bg-gray-200 dark:bg-gray-600 object-cover size-20 sm:size-32 md:size-40`}
              ...
              />
              <X
                onClick={() => {setImages(images.filter((i) => i != img)); setError('') }}
